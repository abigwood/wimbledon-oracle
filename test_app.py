...
class WimbledonOracleTests(unittest.TestCase):
    ...
    def test_html_asset_versions_match(self):
        ...
        self.assertIn("styles.css?v=20260630c", html)
        self.assertIn("app.js?v=20260630c", html)
    ...
    def test_service_worker_updates_app_shell_network_first(self):
        ...
        app = (ROOT / "app.js").read_text()
        self.assertIn("wimbledon-oracle-v34-20260630", sw)
        self.assertIn("styles.css?v=20260630c", sw)
        self.assertIn("app.js?v=20260630c", sw)
        ...
        self.assertIn("if (!stale.length) return", sw)
        self.assertIn("controllerchange", app)
        self.assertIn("showUpdatePrompt", app)
        self.assertIn("appUpdatePrompt", app)
        self.assertIn("registration.update()", app)
        self.assertIn("updateViaCache: \"none\"", app)
        self.assertIn("pendingUpdateReload", app)
        self.assertIn('const APP_BUILD = "20260630c"', app)
    ...
    def test_bottom_nav_uses_shell_footer_not_fixed_overlay(self):
        ...
        self.assertIn("function appScroller", app)
        self.assertIn("function scrollAppToTop", app)
    ...
    def test_live_fixture_loader_uses_worker_with_static_fallback(self):
        ...
        self.assertIn('fetch(`${API}/fixtures?${refresh ? "refresh=1&" : ""}t=${Date.now()}`', app)
        self.assertIn("await loadFixtures(true)", app)
        self.assertIn('fetch(`data/fixtures.json?t=${Date.now()}`', app)
        self.assertIn('if (path === "/fixtures") return getFixtures(env, request);', worker)
        self.assertIn("async function refreshOfficialScores", worker)
        self.assertIn("selectedSettlementCandidates(matchList)", worker)
        self.assertIn('skipped: "no-active-selected-matches"', worker)
        ...
        self.assertIn('crons = ["*/10 * * * *"]', wrangler)
    ...
    def test_league_table_has_movement_arrows_and_whatsapp_export(self):
        ...
        self.assertIn("function drawLeagueTableCard", app)
        self.assertIn("function shareLeagueTableGraphic", app)
        self.assertIn("new File([blob], \"sw19-league-table.png\"", app)
        self.assertIn("navigator.canShare?.({ files: [file] })", app)
        self.assertIn("Building share card...", app)
        self.assertIn("SW19 ORACLE", app)
